...
class GridMap:
    ...
    def random_cell_weighted_by_danger(self, current_pos, max_distance_x, max_distance_y):
        # Création d'une liste contenant toutes les cellules de la grille avec leur poids respectif
        cells = []
        for i in range(max(0, current_pos[0]-max_distance_x), min(self.rows, current_pos[0]+max_distance_x+1)):
            for j in range(max(0, current_pos[1]-max_distance_y), min(self.cols, current_pos[1]+max_distance_y+1)):
                if self.map[i][j] == -1:
                    continue
                weight_sum = 0
                count = 0
...
def merge_grid(grid1, grid2):
    merged_grid = GridMap(grid1.size, grid1.resolution) # Création d'une nouvelle grille vide avec les mêmes paramètres que les deux grilles à fusionner
    
    for i in range(merged_grid.rows):
        for j in range(merged_grid.cols):
            danger1 = grid1.map[i][j]
            danger2 = grid2.map[i][j]
            if danger1 == -1 or danger2 == -1: # Si la même cellule a pour valeurs de danger 1 et -1 dans les deux grilles, sa nouvelle valeur est -1
                merged_grid.map[i][j] = -1
            elif abs(danger1) > abs(danger2): # On attribue la valeur de danger la plus importante en valeur absolue
                merged_grid.map[i][j] = danger1
            else:
                merged_grid.map[i][j] = danger2
                
            # Fusion des listes rescue_centers
            centers1 = set(grid1.rescue_centers)
            centers2 = set(grid2.rescue_centers)
            merged_centers = list(centers1.union(centers2))
            merged_grid.rescue_centers = merged_centers
