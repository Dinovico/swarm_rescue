...
from solutions.grid import GridMap, a_star, a_star_rescue, closest_rescue_center_index
...
class MyDroneV4(DroneAbstract):
    class Activity(Enum):
        """
        All the states of the drone as a state machine
        """
        EXPLORING = 1
        EXPLORING_ODOMETER = 2
        GRASPING_WOUNDED = 3
        GRASPING_WOUNDED_ODOMETER = 4
        DROPPING_AT_RESCUE_CENTER = 5
        DROPPING_AT_RESCUE_CENTER_ODOMETER = 6
    ...
    def control(self):
        ...
        elif self.state is self.Activity.EXPLORING_ODOMETER:
            found_wounded, command = self.control_explore_odometer()
        ...
        # 1 -> 2
        if self.state is self.Activity.EXPLORING and self.gps_is_disabled():
            self.state = self.Activity.EXPLORING_ODOMETER

        # 2 -> 1
        elif self.state is self.Activity.EXPLORING_ODOMETER and not self.gps_is_disabled():
            self.state = self.Activity.EXPLORING

        # 1 -> 3
        elif self.state is self.Activity.EXPLORING and found_wounded:
        ...
        # 2 -> 4
        elif self.state is self.Activity.EXPLORING_ODOMETER and found_wounded:
            self.state = self.Activity.GRASPING_WOUNDED_ODOMETER
        ...
        # 3 -> 4
        elif self.state is self.Activity.GRASPING_WOUNDED and self.gps_is_disabled():
            self.state = self.Activity.GRASPING_WOUNDED_ODOMETER

        # 4 -> 3
        elif self.state is self.Activity.GRASPING_WOUNDED_ODOMETER and not self.gps_is_disabled():
            self.state = self.Activity.GRASPING_WOUNDED
        ...
        # 4 -> 6
        elif self.state is self.Activity.GRASPING_WOUNDED_ODOMETER and self.base.grasper.grasped_entities:
            self.state = self.Activity.DROPPING_AT_RESCUE_CENTER_ODOMETER

        # 5 -> 6
        elif self.state is self.Activity.DROPPING_AT_RESCUE_CENTER and self.gps_is_disabled():
            self.state = self.Activity.DROPPING_AT_RESCUE_CENTER_ODOMETER

        # 6 -> 5
        elif self.state is self.Activity.DROPPING_AT_RESCUE_CENTER_ODOMETER and not self.gps_is_disabled():
            self.state = self.Activity.DROPPING_AT_RESCUE_CENTER
        ...
        # 6 -> 2
        elif self.state is self.Activity.DROPPING_AT_RESCUE_CENTER_ODOMETER and not self.base.grasper.grasped_entities:
            self.state = self.Activity.EXPLORING_ODOMETER
            self.currently_rescuing = False
    ...
    def control_explore_odometer(self):
        command = {"forward": 0.0,
                "lateral": 0.0,
                "rotation": 0.0,
                "grasper": 0}
        ...
        self.last_position = self.last_position + np.array([dist_travel * np.cos(alpha), dist_travel * np.sin(alpha)])
        self.last_angle = (self.last_angle + theta) / 2
        ...
        # Calcul de la direction vers la cible en utilisant les données de l'odomètre
        target_direction = np.array([dist_travel * np.cos(alpha), dist_travel * np.sin(alpha)])
        ...
        if ((np.isnan(self.last_position[0]) or np.isnan(self.last_position[1])) == False):
            ...
            if len(self.path_to_target) > 0:
                ...
                # Calcul de l'intensité de la rotation en fonction de l'angle uniquement
                angle_diff = self.last_angle - angle_to_target
                if angle_diff < -np.pi:
                    angle_diff += 2 * np.pi
                elif angle_diff > np.pi:
                    angle_diff -= 2 * np.pi
                ...
                target_direction = self.grid.gps_to_grid_cell(self.last_position) - self.target_cell
                command["forward"] = min(1, 0.3 + np.linalg.norm(target_direction)/200)
                if np.linalg.norm(target_direction) < 50:
                    self.path_to_target.pop(0)
    ...
    def control_grasp(self):
        ...
        return found_wounded, command_semantic
    ...
    def control_rescue(self):
        ...
        if not self.currently_rescuing:
            ...
            print(self.grid.map)
            print(self.grid.rescue_centers)
            ...
            while(True):
                ...
                print(self.target_cell)
                print(self.grid.map[grid_pos[0]][grid_pos[1]])
