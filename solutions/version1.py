...
class GridMap:
    def __init__(self, size, resolution):
        ...
        self.rows = int(size[0] / resolution)
        self.cols = int(size[1] / resolution)
        self.map = [[-1 for _ in range(self.cols)] for _ in range(self.rows)]
        
    def explore(self, x, y, is_killing_zone):
        if x < 0 or x >= self.rows or y < 0 or y >= self.cols:
            return
        danger = 1 if is_killing_zone else -1
        self.map[x][y] = danger
        for i in range(-5, 6):
            for j in range(-5, 6):
                if i == 0 and j == 0:
                    continue
                self.update_cell_danger(x+i, y+j, danger)
    ...
    def update_cell_danger(self, x, y, danger):
        ...
        self.map[x][y] = max(min(self.map[x][y] + (danger * ((1 - abs(self.map[x][y])) / (distance + 1))), 1), -1)
